...
exports.KeyedBatchedAccumulator = void 0;
 ...
 * is needed.
 */
class KeyedBatchedAccumulator {
    constructor(batchSize) {
        this._keyToAccumulator = new Map();
        if (!isNaturalNumber(batchSize)) {
            // prettier-ignore
            throw new Error(`${KeyedBatchedAccumulator.name} expects a natural number for ` +
     ...
     * @returns The total number of accumulated items across all keys.
     */
    get totalAccumulatedItemsCount() {
        let total = 0;
    ...
    push(item, key) {
        if (typeof key !== 'string' || key === '') {
            // prettier-ignore
            throw new Error(`${KeyedBatchedAccumulator.name} expects a non-empty string as key, ` +
                `but received: '${key}'`);
        }
...
exports.KeyedBatchedAccumulator = KeyedBatchedAccumulator;
...
//# sourceMappingURL=keyed-batched-items-accumulator.js.map